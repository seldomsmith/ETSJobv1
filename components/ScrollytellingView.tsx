...
const chapters = [
  ...
  { id: 'access-gap', title: 'The Accessibility Gap', description: 'Contrasting bounded isochrones measuring reachability of neighborhoods under 30 minute thresholds.', layers: ['isochrones'], viewState: { longitude: -113.4538, latitude: 53.5361, zoom: 11.5, pitch: 20 }, metrics: [{ label: 'Deficit Zones', value: '324 Areas', type: 'pink' }, { label: 'Max Reachable', value: '142k Jobs', type: 'cyan' }] },
      ...
      <div className="sticky top-0 h-screen w-full bg-slate-950">
        <Map {...viewState} onMove={(evt: ViewStateChangeEvent) => setViewState(evt.viewState)} mapboxAccessToken={MAPBOX_TOKEN} mapStyle="mapbox://styles/mapbox/dark-v11" style={{ width: "100vw", height: "100vh" }}>
          ...
          <Source id="routes" type="geojson" data="/data/routes.geojson">
            <Layer id="routes-layer" type="line" layout={{ visibility: activeChap.layers.includes('routes') ? 'visible' : 'none' }} paint={{ 'line-color': '#22d3ee', 'line-opacity': 0.6, 'line-width': 1.5 }} />
          </Source>
